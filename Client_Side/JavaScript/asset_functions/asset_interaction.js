    ...
    xhr.onreadystatechange = function (){
        if(xhr.readyState === 4)
		{
            ...
            else
			{
                ...
                $('#confTxt').html('Created Sample: '+ JSON.parse(array[array.length - 1]).V5CID);
            }
        }
    };
...
function transferAsset()
{
    let found = [];
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets/samples/'+transferArray[transferIndex-1].V5CID+'/owner', true); //PUT (Updates)
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
...
function scrapAsset()
{
	...
	*/
    ...
    let objects = [];
    let xhr = new XMLHttpRequest();
    xhr.open('DELETE', 'blockchain/assets/samples/'+scrapArray[scrapIndex-1].V5CID, true); //As it refers to an v5, the path is: blockchain/assets/samples/vehicle/..
...
let bigData;
let increment;
function updateAsset(bin, type, hiv, name, date, V5CID, el)
{
	/*
	...
	*/
    ...
    bigData = [{'value':bin,'field':'BIN', 'title': 'BIN','V5CID':V5CID},{'value':type,'field':'type', 'title': 'Type','V5CID':V5CID},{'value':hiv,'field':'hiv', 'title': 'Hiv','V5CID':V5CID},{'value':name,'field':'name', 'title': 'Name','V5CID':V5CID},{'value':date,'field':'date', 'title': 'Date','V5CID':V5CID}];
...
function updateField()
{
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets/samples/'+bigData[increment].V5CID+'/'+bigData[increment].field, true);
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
