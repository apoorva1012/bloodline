...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	var V5CIDs V5CID_Holder

	bytes, err := json.Marshal(V5CIDs)

    if err != nil { return nil, errors.New("Error creating V5CID_Holder record") }
...
func (t *SimpleChaincode) retrieve_v5c(stub shim.ChaincodeStubInterface, V5CID string) (Sample, error) {

	var v Sample

	bytes, err := stub.GetState(V5CID);

	if err != nil {	fmt.Printf("RETRIEVE_V5C: Failed to invoke sample_code: %s", err); return v, errors.New("RETRIEVE_V5C: Error retrieving sample with V5CID = " + V5CID) }
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "create_sample" {
		...
		if function == "consume_sample" {																// If its a scrap vehicle then only two arguments are passed (no update value) all others have three arguments and the V5CID is expected in the last argument
			argPos = 0
		}
...
func (t *SimpleChaincode) create_sample(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string, V5CID string) ([]byte, error) {
	var v Sample
	...
	sample_json := "{"+V5C_id+bin+typed+HIV+date+owner+name+leaseContract+status+consumed+"}" 	// Concatenates the variables to create the total JSON object
	...
	matched, err := regexp.Match("^[A-z][A-z][0-9]{7}", []byte(V5CID))  				// matched = true if the V5CID passed fits format of two letters followed by seven digits
												...
												if err != nil { fmt.Printf("CREATE_SAMPLE: Invalid V5CID: %s", err); return nil, errors.New("Invalid V5CID") }

	if 				V5C_id  == "" 	 ||
					matched == false    {
																		fmt.Printf("CREATE_SAMPLE: Invalid V5CID provided");
	...
	err = json.Unmarshal(bytes, &V5CIDs)
																		...
																		if err != nil {	return nil, errors.New("Corrupt V5CID_Holder record") }
	...
	bytes, err = json.Marshal(V5CIDs)
															...
															if err != nil { fmt.Print("Error creating V5CID_Holder record") }
...
func (t *SimpleChaincode) get_samples(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string) ([]byte, error) {
	...
	err = json.Unmarshal(bytes, &V5CIDs)
																			...
																			if err != nil {	return nil, errors.New("Corrupt V5CID_Holder") }
