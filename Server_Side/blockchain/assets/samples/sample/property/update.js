...
let update = function(req, res, next, usersToSecurityContext, property)
{
    ...
    tracing.create('ENTER', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, req.body);

    tracing.create('INFO', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, 'Formatting request');
    res.write('{"message":"Formatting request"}&&');

    return vehicleData.updateAttribute(user_id, functionName, newValue, V5CID)
    .then(function(data) {
        tracing.create('ENTER SUCCESS', 'PUT blockchain/assets/samples/'+V5CID+'/' + property);

        tracing.create('INFO', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, 'Updating '+property+' value');
        res.write('{"message":"Updating owner value"}&&');
        tracing.create('INFO', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, 'Achieving Consensus');
        res.write('{"message":"Achieving Consensus"}&&');
        let result = {};
        result.message = property + ' updated';
        tracing.create('EXIT', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, data);
    ...
    .catch(function(err) {
        ...
        error.error  = true;
        error.message = err;
        tracing.create('ERROR', 'PUT blockchain/assets/samples/'+V5CID+'/' + property, JSON.parse(err));
